...
export class RES extends Component {
  ...
  render() {
    ...
    let contents = this.state.loading ? <p><em>Loading...</em></p> : RES.renderetablissementsTable(this.state.etablissements);
    return (
      <>
      <div className='row'>
        <h1 className='h1'>RES — Indicateurs</h1>
      </div>
  ...
  async fetchByCisco() {
    ...
    this.setState({ loading: true, lastError: null });
    try {
      const response = await axios.get(`qualite/res?cisco=${encodeURIComponent(code)}`);
      this.setState({ etablissements: response.data || [], loading: false });
    } catch (err) {
      console.error(err);
      this.setState({ etablissements: [], loading: false, lastError: err?.message || 'Erreur réseau' });
  ...
  async fetchByEtab() {
    ...
    this.setState({ loading: true, lastError: null });
    try {
      const response = await axios.get(`qualite/res?nomEtab=${encodeURIComponent(name)}`);
      this.setState({ etablissements: response.data || [], loading: false });
    } catch (err) {
      console.error(err);
      this.setState({ etablissements: [], loading: false, lastError: err?.message || 'Erreur réseau' });
