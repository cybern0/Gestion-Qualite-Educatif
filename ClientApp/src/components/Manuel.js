...
export class Manuel extends Component {
  ...
  render() {
    ...
    let contents = this.state.loading ? <p><em>Loading...</em></p> : Manuel.renderetablissementsTable(this.state.etablissements);
    return (
      <>
      <div className='row'>
        <h1 className='h1'>Manuels — Indicateurs</h1>
      </div>
  ...
  async fetchEtabData() {
    ...
    try {
      const response = await axios.get('qualite/manuel');
  ...
  async fetchByCisco() {
    ...
    this.setState({ loading: true, lastError: null });
    try {
      const response = await axios.get(`qualite/manuel?cisco=${encodeURIComponent(code)}`);
      this.setState({ etablissements: response.data || [], loading: false });
    } catch (err) {
      console.error(err);
      this.setState({ etablissements: [], loading: false, lastError: err?.message || 'Erreur réseau' });
  ...
  async fetchByEtab() {
    ...
    this.setState({ loading: true, lastError: null });
    try {
      const response = await axios.get(`qualite/manuel?nomEtab=${encodeURIComponent(name)}`);
      this.setState({ etablissements: response.data || [], loading: false });
    } catch (err) {
      console.error(err);
      this.setState({ etablissements: [], loading: false, lastError: err?.message || 'Erreur réseau' });
